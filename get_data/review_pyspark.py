...
for row in worksheet.iter_rows(min_row=2,max_row=101, values_only=True):
    data_json = {
        "name":"",
        "url":""
    }
...
spark = SparkSession.builder \
    .appName("Get Comments with PySpark") \
    .getOrCreate()

# creat UDF (User Defined Function)
get_comments_udf = udf(get_comments, ArrayType(StringType()))
df = spark.createDataFrame(data)
#use UDF to get get_comments and creat new index
df_with_comments = df.withColumn("comments", get_comments_udf(df["name"],df["url"]))
# print result
df_with_comments.show(100,truncate=False)
# stop SparkSession
spark.stop()
